...
#ifndef QSMS_DOXYMAIN_H
#define QSMS_DOXYMAIN_H

/**
 * \mainpage Quantum Secure Messaging Protocol (QSMS)
 *
 ...
 *
 * \section qsms_sec About QSMS
 *
 * QSMS is a complete specification that not only defines a robust key exchange function but also integrates
 * authentication mechanisms and an encrypted tunnel within a single protocol. Rather than retrofitting existing
 * schemes with quantum-strength primitives, QSMS breaks new ground by introducing an entirely new set of mechanisms
 * designed from the ground up for security and performance in the post-quantum era.
 *
 * \section design_sec Design Philosophy
 *
 * Recognizing that a large-scale migration to post-quantum cryptography is inevitable, QSMS was developed without
 ...
 *
 * QSMS employs state-of-the-art cryptographic algorithms to ensure high levels of security:
 *
 * - **Asymmetric Ciphers:** QSMS supports the Kyber and McEliece asymmetric ciphers with the full range of parameter sets.
 * - **Signature Schemes:** QSMS can use Dilithium or Sphincs+ signature schemes, which were both standardized by NIST.
 * - **Symmetric Cipher:** For symmetric encryption, QSMS uses the authenticated symmetric stream cipher RCS,
 ...
 *
 * QSMS defines two complete protocol specifications that cater to different trust and performance requirements:
 *
 ...
 *
 * QSMS offers a modern, flexible, and secure alternative to traditional key exchange protocols that are now being
 * retrofitted with quantum-safe algorithms. Designed specifically for the post-quantum era, QSMS integrates robust
