...
static void qsmp_client_dispose(qsmp_kex_client_state* ctx)
{
	assert(ctx != NULL);
...
static void qsmp_client_kex_reset(qsmp_kex_client_state* ctx)
{
	qsc_memutils_clear(ctx->config, sizeof(ctx->config));
	qsc_memutils_clear(ctx->keyid, sizeof(ctx->keyid));
...
static void qsmp_client_initialize(qsmp_kex_client_state* ctx, const qsmp_client_key* ckey)
{
	assert(ckey != NULL);
	assert(ctx != NULL);
	...
	if (ckey != NULL && ctx != NULL)
	{
		...
		ctx->exflag = qsmp_message_none;
	}
}

static qsmp_errors qsmp_client_connection_request(qsmp_kex_client_state* ctx, qsmp_packet* packetout)
{
	assert(ctx != NULL);
	...
	if (ctx != NULL && packetout != NULL)
	{
		...
		if (tm <= ctx->expiration)
		{
			...
			if (qsc_acp_generate(ctx->token, QSMP_STOKEN_SIZE) == true)
			{
				...
				packetout->flag = qsmp_message_connect_request;
				...
				qsc_sha3_update(&kstate, keccak_rate_256, packetout->message, QSMP_STOKEN_SIZE + QSMP_CONFIG_SIZE);
				qsc_sha3_update(&kstate, keccak_rate_256, ctx->verkey, QSMP_VERIFYKEY_SIZE);
				qsc_sha3_finalize(&kstate, keccak_rate_256, ctx->pkhash);

				qerr = qsmp_error_none;
				ctx->exflag = qsmp_message_connect_request;
			}
			else
			{
				ctx->exflag = qsmp_message_none;
...
static qsmp_errors qsmp_client_exstart_request(qsmp_kex_client_state* ctx, const qsmp_packet* packetin, qsmp_packet* packetout)
{
	assert(ctx != NULL);
	...
	{
		if (ctx->exflag == qsmp_message_connect_request && packetin->flag == qsmp_message_connect_response)
		{
			slen = 0;
			...
			if (qsmp_signature_verify(khash, &slen, packetin->message, mlen, ctx->verkey) == true)
			{
				...
				qsc_memutils_copy(pubk, ((uint8_t*)packetin->message + mlen), QSMP_PUBLICKEY_SIZE);
				...
				if (qsc_intutils_verify(phash, khash, QSC_SHA3_256_HASH_SIZE) == 0)
				{
					...
					qsc_cshake_initialize(&kstate, keccak_rate_256, sec, QSMP_SECRET_SIZE, NULL, 0, ctx->pkhash, QSMP_PKCODE_SIZE);
					...
					packetout->flag = qsmp_message_exstart_request;
					packetout->msglen = QSMP_CIPHERTEXT_SIZE;
					packetout->sequence = ctx->txseq;

					qerr = qsmp_error_none;
					ctx->exflag = qsmp_message_exstart_request;
				}
				else
				{
					qsmp_packet_error_message(packetout, qsmp_error_hash_invalid);
			...
			{
				qsmp_packet_error_message(packetout, qsmp_error_auth_failure);
		...
		{
			qsmp_packet_error_message(packetout, qsmp_error_invalid_request);
			ctx->exflag = qsmp_message_none;
...
static qsmp_errors qsmp_client_exchange_request(qsmp_kex_client_state* ctx, const qsmp_packet* packetin, qsmp_packet* packetout)
{
	assert(ctx != NULL);
	assert(packetin != NULL);
	assert(packetout != NULL);
	...
	if (ctx != NULL && packetin != NULL && packetout != NULL)
	{
		if (ctx->exflag == qsmp_message_exstart_request && packetin->flag == qsmp_message_exstart_response)
		{
			...
			qsc_acp_generate(ctx->mackey, QSMP_MACKEY_SIZE);
			...
			qsc_memutils_clear(packetout->message, sizeof(packetout->message));
			packetout->flag = qsmp_message_exchange_request;
			...
			qerr = qsmp_error_none;
			ctx->exflag = qsmp_message_exchange_request;
...
static qsmp_errors qsmp_client_establish_request(qsmp_kex_client_state* ctx, const qsmp_packet* packetin, qsmp_packet* packetout)
{
	assert(ctx != NULL);
	assert(packetin != NULL);
	assert(packetout != NULL);
	...
	if (ctx != NULL && packetin != NULL && packetout != NULL)
	{
		if (ctx->exflag == qsmp_message_exchange_request && packetin->flag == qsmp_message_exchange_response)
		{
			uint8_t kcode[QSMP_MACTAG_SIZE] = { 0 };

			/* mac the cipher-text */
			qsc_kmac256_compute(kcode, QSMP_MACTAG_SIZE, ((uint8_t*)packetin->message + QSMP_MACTAG_SIZE), QSMP_CIPHERTEXT_SIZE, ctx->mackey, QSMP_MACKEY_SIZE, NULL, 0);
			...
			if (qsc_intutils_verify(packetin->message, kcode, QSMP_MACTAG_SIZE) == 0)
			{
				...
				if (qsmp_cipher_decapsulate(sec, ((uint8_t*)packetin->message + QSMP_MACTAG_SIZE), ctx->prikey) == true)
				{
					...
					qsc_memutils_clear(packetout->message, sizeof(packetout->message));
					packetout->flag = qsmp_message_establish_request;
					packetout->msglen = 0;
					packetout->sequence = ctx->txseq;

					qerr = qsmp_error_none;
					ctx->exflag = qsmp_message_session_established;
				}
				else
				{
					qsmp_packet_error_message(packetout, qsmp_error_decapsulation_failure);
...
static qsmp_errors qsmp_client_kex(qsmp_kex_client_state* ctx, qsc_socket* sock, const qsmp_client_key* ckey)
{
	...
	qsmp_client_initialize(ctx, ckey);
	/* create the connection request packet */
	qerr = qsmp_client_connection_request(ctx, &reqt);
		...
		if (slen == plen + QSC_SOCKET_TERMINATOR_SIZE)
		{
			...
			if (rlen == QSMP_EXSTART_RESPONSE_SIZE + QSC_SOCKET_TERMINATOR_SIZE)
			{
				...
				if (resp.sequence == ctx->rxseq)
				{
					...
					if (resp.flag == qsmp_message_exstart_response)
					{
						qsmp_packet_clear(&reqt);
						/* create the exchange request packet */
						qerr = qsmp_client_exchange_request(ctx, &resp, &reqt);
					}
					else
					{
						if (resp.flag == qsmp_message_error_condition)
		...
		if (slen == plen + QSC_SOCKET_TERMINATOR_SIZE)
		{
			...
			if (rlen == QSMP_EXCHANGE_RESPONSE_SIZE + QSC_SOCKET_TERMINATOR_SIZE)
			{
				...
				if (resp.sequence == ctx->rxseq)
				{
					...
					if (resp.flag == qsmp_message_exchange_response)
					{
						qsmp_packet_clear(&reqt);
						/* create the establish request packet */
						qerr = qsmp_client_establish_request(ctx, &resp, &reqt);
					}
					else
					{
						if (resp.flag == qsmp_message_error_condition)
	...
	if (qerr == qsmp_error_none)
	{
		...
		if (slen == plen + QSC_SOCKET_TERMINATOR_SIZE)
		{
			...
			if (rlen != QSMP_ESTABLISH_RESPONSE_SIZE + QSC_SOCKET_TERMINATOR_SIZE || (qsmp_flags)resp.message[0] != qsmp_message_session_established)
			{
				qerr = qsmp_error_receive_failure;
			}
			else
...
void qsmp_client_decode_public_key(qsmp_client_key* clientkey, const char input[QSMP_PUBKEY_STRING_SIZE])
{
	...
	if (clientkey != NULL)
	{
		...
		qsc_intutils_hex_to_bin(((char*)input + spos), clientkey->keyid, QSMP_KEYID_SIZE * 2);
		...
		qsc_stringutils_remove_line_breaks(tmpvk, sizeof(tmpvk), ((char*)input + spos), (QSMP_PUBKEY_STRING_SIZE - (spos + sizeof(QSMP_PUBKEY_FOOTER))));
		qsc_encoding_base64_decode(clientkey->verkey, QSMP_VERIFYKEY_SIZE, tmpvk, QSMP_PUBKEY_ENCODING_SIZE);
	}
}
...
void qsmp_client_send_error(qsc_socket* sock, qsmp_errors error)
{
	...
	{
		resp.flag = qsmp_message_error_condition;
		resp.sequence = 0xFF;
		resp.msglen = 1;
...
void qsmp_client_connection_close(qsmp_kex_client_state* ctx, qsc_socket* sock, qsmp_errors error)
{
	if (qsc_socket_is_connected(sock) == true)
	{
		...
		resp.flag = qsmp_message_connection_terminate;
...
qsmp_errors qsmp_client_connect_ipv4(qsmp_kex_client_state* ctx, qsc_socket* sock, const qsmp_client_key* ckey, const qsc_ipinfo_ipv4_address* address, uint16_t port)
{
	...
	{
		qerr = qsmp_client_kex(ctx, sock, ckey);
	}
	else
	{
		qerr = qsmp_error_connection_failure;
...
qsmp_errors qsmp_client_decrypt_packet(qsmp_kex_client_state* ctx, const qsmp_packet* packetin, uint8_t* message, size_t* msglen)
{
	...
	if (ctx != NULL && message != NULL && msglen != NULL && packetin != NULL)
	{
		...
		{
			if (ctx->exflag == qsmp_message_session_established)
			{
			...
			else if (ctx->exflag != qsmp_message_keep_alive_request)
			{
				*msglen = 0;
...
qsmp_errors qsmp_client_encrypt_packet(qsmp_kex_client_state* ctx, const uint8_t* message, size_t msglen, qsmp_packet* packetout)
{
	...
	if (ctx != NULL && packetout != NULL && message != NULL)
	{
		if (ctx->exflag == qsmp_message_session_established)
		{
			...
			ctx->txseq += 1;
			qsc_memutils_clear(packetout->message, sizeof(packetout->message));
			packetout->flag = qsmp_message_encrypted_message;
